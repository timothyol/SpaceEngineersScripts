    ...
    class Program
    {
        ...
        ****************Copy from below this comment************************/
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            GridTerminalSystem.GetBlocksOfType(asses);
            ...
            var itemsToQueue = new List<ItemAndAmount>();
            ...
            for(int i = 0; i < ItemMinimums.Count; ++i)
            {
                ...
                if (needCount > 0)
                {
                    ...
                    itemsToQueue.Add(new ItemAndAmount { bpName = bpName, needCount = needCount, haveCount = haveCount });
                }
            }

            itemsToQueue.Sort(NeedItemCompare);

            if(primeAss != null)
                for(int i = 0; i < itemsToQueue.Count; ++i)
                {
                    var t = itemsToQueue[i];
                    var bp = GetBlueprint(t.bpName);
                    primeAss.AddQueueItem(bp, (VRage.MyFixedPoint)t.needCount);
                }
        }

        private int NeedItemCompare(ItemAndAmount arg1, ItemAndAmount arg2)
        {
            return arg1.haveCount - arg2.haveCount;
        }
